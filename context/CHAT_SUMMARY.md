...
- **Страницы `/privacy` и `/terms`**: `frontend/src/pages/legal/` (PrivacyPage, TermsPage,
  общий LegalLayout). Роуты standalone (без Layout), доступны гостям. В текстах TODO —
  заполнить ФИО/ИНН самозанятого. Русские версии; EN — в бэклоге.
- **Чекбокс согласия**: AuthPage, режим регистрации — обязательный чекбокс со ссылками
  на /terms и /privacy (кнопка disabled без галочки). Для Google и телефона — строка
  «Продолжая, вы принимаете…» (ConsentNote). Ключи: auth.json `consent.*` (ru+en).
- **TASKS.md**: раздел «Юридическое (152-ФЗ)» — уведомление РКН, локализация БД
  (план: Timeweb Cloud DBaaS + S3), трансграничная передача, удаление аккаунта.
- Переезд БД на Timeweb обсуждён и отложен (код отката — upload.js остался на Supabase).
...
- **dishes.js — оптимизация**: fridgeMode и recommendations переведены с «загрузить все
  блюда и фильтровать в JS» на Prisma-фильтры (some/none по обязательным ингредиентам).
  nearMatch: лёгкий запрос id+ингредиенты → догрузка 6 полных блюд. Семантика сохранена.
